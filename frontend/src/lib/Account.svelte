<script>
  ...
  async function getExistingSlots() {
    ...
    // Convert map back to array and update store once everything is fetched
    const newSlots = Array.from(currentSlotsMap.values());
    slots.set(newSlots);

    // Preserve selected address if it still exists
    const selected = newSlots.find((s) => s.email === $currentAddress.email);
    if (selected) {
  ...
  async function generateNewSlot() {
    ...
    if (!res.ok) {
      if (res.status == 401) return showPopup("You're Unauthorized to do this");
      if (res.status == 429)
      ...
      <div class="account-header">
        ...
        <div class="email-slots-grid" id="emailSlotsGrid">
          {#each $slots as slot (slot.id)}
            <div
              class="email-slot {slot.email === selectedAddress.email
                ? 'active'
                : ''}"
            ...
            >
              <div class="email-slot-header">
                ...
                <div class="email-slot-actions">
                  <button
                    on:click|stopPropagation={() => copyAddress(slot.email)}
                    class="slot-action-btn"
                    title="Copy email">📋</button
                  >
                  <button
                    on:click|stopPropagation={() => deleteEmailSlot(slot.email)}
                    class="slot-action-btn"
              ...
              <div class="email-slot-info">
                <span class="email-count">{slot.emails.length} emails</span>
              </div>
            </div>
          {/each}
        </div>
